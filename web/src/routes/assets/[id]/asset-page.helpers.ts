...
export function segmentMediaStart(
  segment: Pick<TranscriptSegment, "start" | "chunk_start">,
) {
  return typeof segment.chunk_start === "number" &&
    Number.isFinite(segment.chunk_start)
    ? segment.chunk_start
    : segment.start;
}

export function segmentMediaEnd(
  segment: Pick<TranscriptSegment, "end" | "chunk_end">,
) {
  return typeof segment.chunk_end === "number" &&
    Number.isFinite(segment.chunk_end)
    ? segment.chunk_end
    : segment.end;
}
...
export function activeTranscriptSegmentIndex(
  ...
  return segments.findIndex(
    (segment) =>
      time >= segmentMediaStart(segment) && time < segmentMediaEnd(segment),
  );
}
...
export function localSpeakerRows(asset: AssetDetail): LocalSpeakerRow[] {
  ...
  for (const speaker of Object.keys(asset.speaker_centroids ?? {}))
    speakers.add(speaker);
  for (const segment of asset.transcript_segments ?? [])
    speakers.add(segment.speaker);

  return [...speakers].sort().map((localSpeaker) => {
    const segments =
      asset.transcript_segments?.filter(
        (segment) => segment.speaker === localSpeaker,
      ) ?? [];
    const named = segments.find(
      (segment) =>
        segment.speaker_name && segment.speaker_name !== localSpeaker,
    );
    const matched = segments.find(
      (segment) => segment.speaker_id && segment.speaker_id !== localSpeaker,
    );
    return {
      localSpeaker,
      ...
      firstStart: segments.length
        ? Math.min(...segments.map((segment) => segmentMediaStart(segment)))
        : null,
      lastEnd: segments.length
        ? Math.max(...segments.map((segment) => segmentMediaEnd(segment)))
        : null,
    };
  });
}
...
export function thumbnailUrl(assetId: string, event: VisualEvent) {
  return authenticatedResourceUrl(
    `/api/assets/${assetId}/visual-events/${event.event_index}/thumbnail`,
  );
}
...
export function clampMediaPaneWidth(width: number, availableWidth: number) {
  const maxWidth = Math.max(
    MEDIA_PANE_MIN_WIDTH,
    availableWidth - TRANSCRIPT_PANE_MIN_WIDTH,
  );
