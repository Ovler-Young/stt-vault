...
def _upload_destination(data_media_dir: Path, filename: str) -> tuple[str, Path, Path, str]:
...
def store_upload(data_media_dir: Path, filename: str, source_path: Path) -> tuple[str, Path, str]:
    asset_id, target_dir, stored_path, media_type = _upload_destination(data_media_dir, filename)
