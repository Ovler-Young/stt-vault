...
def test_batched_diarization_instruments_each_provider_operation(tmp_path: Path) -> None:
    ...
    class BatchedProvider:
        ...
        def _perform_vad(self, _wav_path: str) -> list[dict[str, float]]:
            return [{"start": 0.0, "end": 1.0}]

        def _generate_subsegments(
            self, vad_segments: list[dict[str, float]], _accurate: bool | None
        ) -> list[dict[str, float]]:
            return vad_segments

        def _extract_fbank_features(
            self, _wav_path: str, _subsegments: list[dict[str, float]]
        ...
        def _perform_clustering(
            self, _embeddings: np.ndarray, _subsegments: list[dict[str, float]]
    ...
    manager = DiarizerManager(device="cpu", idle_timeout_seconds=1)
    manager._instrument_diarizer(provider)

    result = manager._diarize_batched(provider, str(wav_path))

    assert result is not None
    assert result["raw_speakers_detected"] == 1
    assert result["merged_speakers_detected"] == 1
